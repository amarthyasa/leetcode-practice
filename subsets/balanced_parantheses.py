...
def generate_valid_parentheses(num):
    result = []
    queue = deque()
    queue.append(PranthesesString("", 0,0))
    while queue:
      ps = queue.popleft()
      if ps.open_count == num and ps.close_count == num:
            ...
            if ps.open_count < num:
                  queue.append(PranthesesString(ps.str + "(", ps.open_count+1, ps.close_count))
            if ps.close_count < ps.open_count:
                  queue.append(PranthesesString(ps.str + ")", ps.open_count, ps.close_count+1))
